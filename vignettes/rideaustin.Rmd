...
for (t in 2:time_periods) {
  mods_cor[[t]] <- ddpn_mix(mods_cor[[t - 1]]$updated_model, data_list[[t]])
}
```
...
for (t in 1:time_periods) {
  x <- data_list[[t]]
  density <- ddpn_eval(mods_cor[[t]]$updated_model, data.matrix(mesh), nparticles = 100)
